...
try:
    arduino = serial.Serial(port='COM9', baudrate=9600 , timeout=1)  # Update 'COM3' to your Mega's port
    time.sleep(2)  # Wait for connection
    logging.info("Connected to Arduino Mega")
except Exception as e:
    logging.error(f"Failed to connect to Arduino: {e}")
    messagebox.showerror("Error", f"Arduino connection failed: {e}")
    arduino = None
...
class AttendanceSystem:
    def __init__(self, root):
        ...
        # Start camera and Arduino listener
        self.start_camera()
        self.listen_arduino()
    ...
    def listen_arduino(self):
        """Listen for commands from Arduino Mega."""
        if not self.arduino or not self.camera_running:
            self.root.after(100, self.listen_arduino)
            return
        ...
        try:
            if self.arduino.in_waiting > 0:
                ...
                if data == "CAPTURE":
                    logging.info("Received CAPTURE command from Arduino")
    ...
    def start_capture(self):
        ...
        if not name:
            ...
            if self.arduino:
                self.arduino.write(b'E')  # Send error to Arduino
            return
                ...
                if self.capturing and self.capture_count < 30 and not self.capture_cancelled:
                    ...
                    if self.capture_count >= 30:
                        if self.verify_captured_images():
                            self.finish_capture()
                            if self.arduino:
                                self.arduino.write(b'S')  # Send success to Arduino
                        else:
    ...
    def exit_app(self):
        ...
        if self.cap:
            self.cap.release()
        if self.arduino:
            self.arduino.close()  # Close serial connection
